...
def enviar_codigo():
    ...
    #try:
        #geo_resposta = requests.get(f"https://ipapi.co/{ip}/json/")
        #if geo_resposta.status_code == 200:
            #geo = geo_resposta.json()
            #cidade = geo.get("city", "")
            #regiao = geo.get("region", "")
           # pais = geo.get("country_name", "")
            #localizacao = f"{cidade}, {regiao}, {pais}".strip(", ")
        #else:
          #  localizacao = "Localização desconhecida"
    #except Exception as e:
     #   print("Erro ao obter geolocalização:", e)
    #    localizacao = "Erro na geolocalização"
